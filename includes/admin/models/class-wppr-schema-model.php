...
class WPPR_Schema_Model extends WPPR_Model_Abstract {
	 ...
	 *
	 * @var array $dataTypesAllowed The schema data types allowed to become fields.
	 */
	private $dataTypesAllowed;

	/**
	 * WPPR_Review_Type_Model constructor.
	 *
	 * @since   3.5.0
	 * @access  public
	 *
	...
	private function setup_types( array $things ) {
		$this->dataTypesAllowed	= apply_filters( 'wppr_schema_data_types_allowed', array( 'schema:Text', 'schema:URL', 'schema:Date', 'schema:Number', 'schema:Boolean', 'schema:DateTime', 'schema:Time', 'schema:Integer', 'schema:Float', 'schema:False', 'schema:True', 'schema:QuantitativeValue', 'schema:Distance' ) );
	...
	private function extract_if( $element, $extract, $key, $val ) {
		...
		if ( is_array( $extracted ) ) {
			$rangeIncludes	= $extracted['schema:rangeIncludes'];
			$dataTypes		= array_values( $rangeIncludes );
			if ( count( $dataTypes ) > 1 ) {
				$dataTypes		= array_values( wp_list_pluck( $dataTypes, '@id' ) );
			}
			if ( empty( $common = array_intersect( $this->dataTypesAllowed, $dataTypes ) ) ) {
				$extracted = null;
			}
		}
